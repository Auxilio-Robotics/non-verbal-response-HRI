...
function renderpupil(r, theta, offsets){
    ...
    eyeLoc[0] = Math.min(r, 178) * Math.cos(theta);
    eyeLoc[1] = Math.min(r, 178) * Math.sin(theta);
    ...
    ctx.fillStyle = "white";  
    ctx.beginPath();
    ctx.arc(xoffset + 30 + eyeLoc[0]+ offsets[0], yoffset - 30 + eyeLoc[1]+ offsets[1], 7, 0, Math.PI * 2);
    ctx.fill();
...
function animate(){
    ...
    ctx.clip();    
    renderpupil(eyepos[0], eyepos[1], states[curstate]['offsets']);
